...
class Pagination  //ESTA PAGINACION USA LIMIT SI DESEA USAR OTRA BD DIFERENTE DE MYSQL CAMBIE LA LINEA(34) CORRESPONDIENTE
{
	protected $tabla;
	protected $xpags;
	protected $page;
	...
	public function paginar(){
		...
		$model = new Model();
		$query_pag_data = "SELECT * FROM " . $this->tabla . " WHERE categoria_id = '".$this->cat."' ORDER BY position DESC LIMIT $start, $per_page"; //SELECT CON LIMITES
		...
		$query_pag_num = "SELECT * FROM " . $this->tabla." WHERE categoria_id = '".$this->cat."' ORDER BY position ASC"; //CUENTA LOS REGISTROS
